...
if(isset($_POST["submit"])){
    ...
    $removeType = mysqli_real_escape_string($connection, $_POST["remove_type"]);
    ...
    if($existingReturnedRows != 1){
        ...
        header("Location: ../Products/removeStock.php?removeStock=Invalid");
    }
    else{
        ...
        if($currentAmount[2] >= $productAmount){
            ...
            createEvent($connection, $_SESSION['$ses_user_username'], $productName, (int)$productAmount, $removeType);
            header("Location: ../Products/removeStock.php?removeStock=Success");
        }
        else{
            header("Location: ../Products/removeStock.php?removeStock=StockUnavailable");
        }
    }
}
else{
